...
async function verifyRecaptcha(token, expectedAction, minimumScore = 0.5) {
  try {
    ...
    if (data.score !== void 0 && data.score < minimumScore) {
      return {
        success: false,
        score: data.score,
...
async function sendBookingFormEmail(data) {
  try {
    const businessEmail = await getResendClient().emails.send({
      from: serverEnv.email.fromEmail,
      to: serverEnv.email.toEmail,
...
function toEasternTime(isoString) {
...
function generateContactFormEmailHTML(data) {
  const leadData = data.sessionData;
  const submissionTime = toEasternTime((/* @__PURE__ */ new Date()).toISOString());
  return `
    <!DOCTYPE html>
    <html>
    <head>
...
function generateLeadTrackingHTML(sessionData) {
  return `
    <!-- Lead Intelligence -->
...
function generateContactFormAutoReplyHTML(data) {
  return `
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <div style="background: #F3F3E6; padding: 40px 30px;">
          ...
          <div style="background: white; border-radius: 12px; padding: 25px; border-left: 5px solid #BB8525; margin-bottom: 30px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
              <a href="${serverEnv.business.website}" style="background: #F8F9FA; border: 2px solid #D2A63C; padding: 15px; border-radius: 8px; text-decoration: none; text-align: center; transition: all 0.3s ease;">
              ...
              <a href="${serverEnv.business.website}/services" style="background: #F8F9FA; border: 2px solid #EECD5C; padding: 15px; border-radius: 8px; text-decoration: none; text-align: center; transition: all 0.3s ease;">
            ...
            <div style="text-align: center; margin-top: 15px;">
              <a href="${serverEnv.business.website}/reviews" style="background: #BB8525; color: white; padding: 12px 25px; border-radius: 6px; text-decoration: none; font-weight: 500; font-size: 14px; display: inline-block;">
...
function generateBookingFormEmailHTML(data) {
  const urgencyColors = {
    "same-day": "#D2A63C",
    "next-day": "#EECD5C",
    "flexible": "#BB8525"
...
function generateBookingFormAutoReplyHTML(data) {
  const urgencyLabels = {
    "same-day": "Same Day Service",
    "next-day": "Next Day Service",
    "flexible": "Flexible Scheduling"
        ...
        <div style="background: #F3F3E6; padding: 40px 30px;">
          ...
          <div style="background: white; border-radius: 12px; padding: 25px; border-left: 5px solid #BB8525; margin-bottom: 30px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
              ...
              <a href="${serverEnv.business.website}/faq" style="background: #F8F9FA; border: 2px solid #EECD5C; padding: 15px; border-radius: 8px; text-decoration: none; text-align: center; transition: all 0.3s ease;">
...
export {
  sendContactFormEmail as a,
  sendBookingFormEmail as s,
  verifyRecaptcha as v
};
